...
class SnifferCollector(Notifications.Notifier):
                ...
                if packet.blePacket.accessAddress == ADV_ACCESS_ADDRESS:    
                    ...
                    if self.state == STATE_SCANNING:
                        if                 (packet.blePacket.advType == 0 
                            ...
                            #print "AdvType: %d" % packet.blePacket.advType
                            #addressString = ":".join(['%02X' % a for a in packet.blePacket.advAddress])
                            #print "AdvAddress: %s" % addressString
                            newDevice = Devices.Device(address=packet.blePacket.advAddress, name=packet.blePacket.name, RSSI=packet.RSSI, carName=packet.blePacket.carName, txAdd=packet.txAdd)
    ...
    def _startFollowing(self, device, followOnlyAdvertisements = False):
        ...
        response = self._packetReader.sendFollow(device.address, device.txAdd, followOnlyAdvertisements)
    ...
    def _sendTestPacket(self, payload):
        #JRG
        self._packetReader.sendPacket(TEST_COMMAND_ID,[])
