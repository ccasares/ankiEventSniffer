...
#        packetList = [HEADER_LENGTH] + [len(payload)] + [PROTOVER] + toLittleEndian(self.packetCounter, 2) + payload
        packetString = listToString(self.encodeToSLIP(packetList))
        self.packetCounter += 1
        x = self.uart.writeList(packetString, timeout)
        return x
    ...
    def sendFollow(self, addr, txAdd=1, followOnlyAdvertisements = False, timeout = None):
        ...
        # addr.append(txAdd)
        response = self.sendPacket(REQ_FOLLOW, addr+[followOnlyAdvertisements], timeout)
        return response
...
class BlePacket():
    def __init__(self, packetList):
        #print "JRG"
        #pl = " ".join(['%02X' % a for a in packetList])
        #print pl
        ...
        if self.accessAddress == ADV_ACCESS_ADDRESS:    
            ...
            self.extractAnkiCarname(packetList)
    ...
    # JRG
    def extractAnkiCarname(self, packetList):
        ...
        if length > 38:
          ...
          if carId == 1:
            ...
            self.carName = "Guardian"
          elif carId == 14:
            self.carName = "Big Bang"
          else:
            # print "CarID: %d" % carId
            self.carName = "Unknown"
          # print "Car Name: %s" % self.carName
